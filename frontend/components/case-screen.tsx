...
export function CaseFacts({ caseRecord }: { caseRecord: CaseRecord }) {
  ...
  return (
    <section className="case-facts" aria-labelledby="terms-title">
      ...
      <div className="terms-copy"><article><h3>Vulnerability</h3><p>{caseRecord.vulnerability_spec}</p></article><article><h3>Acceptance criteria</h3><p>{caseRecord.acceptance_criteria}</p></article></div>
      <div className="review-paths"><h3>Review paths</h3><ul>{caseRecord.review_paths.map((path) => <li key={path}><code>{path}</code></li>)}</ul></div>
    </section>
  );
}
...
function ActionPanel({ caseRecord, nowSeconds, onReload }: { caseRecord: CaseRecord; nowSeconds: number; onReload(): Promise<void> }) {
  ...
  const execute = async (execution: ActionExecution) => {
    ...
    try {
      const result = await tx.run({
        ...
        precondition: async () => {
          const fresh = await readCase(caseRecord.case_id);
          if (fresh.status !== execution.expectedStatus) throw new Error(`Case state changed to ${fresh.status}`);
          if (execution.role && wallet.address?.toLowerCase() !== fresh[execution.role === "client" ? "client_address" : "developer_address"].toLowerCase()) throw new Error(`Connected wallet is not the ${execution.role}`);
          if (execution.deadline !== undefined && !isDeadlineOpen(Math.floor(Date.now() / 1_000), execution.deadline)) throw new Error("The action window has closed");
        },
      });
  ...
  const validateInputs = () => {
    if (isShaAction && !validateCommitSha(sha)) return "Commit must be exactly 40 lowercase hexadecimal characters.";
    if (action === "CHALLENGE") return validateChallengePath(challengePath) ?? "";
    return "";
  };
  const submit = (event: React.FormEvent) => {
    event.preventDefault();
    const validation = validateInputs();
    if (validation) { setInputError(validation); return; }
    const common = { expectedStatus: caseRecord.status } as const;
    if (action === "ACCEPT") void execute({ ...common, method: "accept_case", args: [caseRecord.case_id], expected: { allowedStatuses: ["ACCEPTED"] }, role: "developer" });
    if (action === "SUBMIT_PATCH") void execute({ ...common, method: "submit_patch", args: [caseRecord.case_id, sha], expected: { allowedStatuses: ["ACCEPTED", "PROVISIONAL_FIXED"], previousActiveSubmissionId: caseRecord.active_submission_id }, role: "developer" });
    if (action === "CHALLENGE") void execute({ ...common, method: "challenge", args: [caseRecord.case_id, sha, challengePath], expected: { allowedStatuses: ["CHALLENGED"] }, role: "client", deadline: caseRecord.challenge_deadline });
    if (action === "RESPOND") void execute({ ...common, method: "respond_to_challenge", args: [caseRecord.case_id, sha], expected: { allowedStatuses: ["CHALLENGED", "FINALIZED_DEVELOPER", "FINALIZED_CLIENT"], previousActiveSubmissionId: caseRecord.active_submission_id, settlementStatuses: ["FINALIZED_DEVELOPER", "FINALIZED_CLIENT"] }, role: "developer", deadline: caseRecord.response_deadline });
    if (action === "FINALIZE") void execute({ ...common, method: "finalize_uncontested", args: [caseRecord.case_id], expected: { allowedStatuses: ["FINALIZED_DEVELOPER"], requiresSettlement: true, settlementRecipient: caseRecord.developer_address } });
    if (action === "REFUND") void execute({ ...common, method: "authorize_client_refund", args: [caseRecord.case_id], expected: { allowedStatuses: ["FINALIZED_CLIENT"], requiresSettlement: true, settlementRecipient: caseRecord.client_address } });
  };

  const actionCopy: Record<CaseAction, [string, string, string]> = {
    ACCEPT: ["Accept case", "Accept immutable terms", "Acceptance changes no term and unlocks patch submission."],
    SUBMIT_PATCH: ["Submit patch", "Submit exact patch commit", "PatchBond retrieves the committed repository and exact commit itself."],
    CHALLENGE: ["Challenge result", "Submit repository-bound challenge", "The artifact is authenticated to this repository and commit; validators still judge its substance."],
    RESPOND: ["Respond", "Submit response patch", "The response remains bound to the original vulnerability and acceptance criteria."],
    FINALIZE: ["Finalize fix", "Authorize developer settlement", "The challenge window has closed. Finalization selects the developer and exact bounty."],
...
export function CaseScreen({ caseId, verify = false }: { caseId: string; verify?: boolean }) {
  ...
  const load = useCallback(async () => {
    if (!validateCaseId(caseId)) { setError("Invalid case ID. Use the exact lowercase identifier."); setLoading(false); return; }
    setLoading(true); setError("");
    try {
      const record = await readCase(caseId);
      const history = await readSubmissionHistory(record);
      setCaseRecord(record); setSubmissions(history);
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Case could not be read"); }
